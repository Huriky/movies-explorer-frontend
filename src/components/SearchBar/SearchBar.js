...
export const SearchBar = ({onSubmit, onChecked, defaultValue}) => {
    ...
    <section className={'searchbar'}>
      <div className="searchbar__container">
        <form onSubmit={handleSubmit} className={`searchbar__content ${focused ? 'focused' : ''}`}>
          ...
          <input defaultValue={defaultValue} name="request" onChange={handleChange} value={data?.request ?? defaultValue } onFocus={() => setFocus(true)} onBlur={() => setFocus(false)} placeholder={'Фильмы'} type="text" className={'searchbar__input'}/>
